...
for j in teams:
    ...
    ) == games_per_team, f"max_games_per_team_{j}"
...
for j in ["Team 4"]:
    prob += (
        (
            lpSum([slots_vars[i, j, k] for i in dow_slots] for k in teams)  # home team on tuesday
            + lpSum([slots_vars[i, k, j] for i in dow_slots] for k in teams) # away team on tuesday
        )
        == 1,
        f"team4Weds{j}",
    )
...
# Team 8 no Sunday mornings
dow_slots = working_slots[working_slots["Day_of_Week"] == "Sunday"].index
am_times = ["9:00", "09:30", "10:00", "10:30", "11:00", "11:30"]
am_slots = working_slots[working_slots["Start"].isin(am_times)].index
sunday_mornings = list(set(dow_slots).intersection(am_slots))
for j in ["Team 8"]:
    prob += (
        (
            lpSum([slots_vars[i, j, k] for i in sunday_mornings] for k in teams)  # home team on tuesday
            + lpSum([slots_vars[i, k, j] for i in sunday_mornings] for k in teams) # away team on tuesday
        )
        == 0,
        f"sunday_mornings{j}",
    )

# Team 3 after 1pm on 3/5, 3/12, 3/19
...
date_am_slots = list(set(date_slots).difference(am_slots))
for j in ["Team 3"]:
    prob += (
        (
            lpSum([slots_vars[i, j, k] for i in date_am_slots] for k in teams)  # home team on tuesday
            + lpSum([slots_vars[i, k, j] for i in date_am_slots] for k in teams) # away team on tuesday
        )
        == 0,
        f"team3_ams{j}",
    )
...
# Team 4 minimal between 4/26 and 4/30
...
for j in ["Team 4"]:
    prob += (
        (
            lpSum([slots_vars[i, j, k] for i in date_slots] for k in teams)  # home team on tuesday
            + lpSum([slots_vars[i, k, j] for i in date_slots] for k in teams) # away team on tuesday
        )
        == 0,
        f"team4_4_26_30{j}",
    )
